    ...
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <span className="flex items-center gap-2">
          {selectedCategory ? (
            <>
              <span className="text-green-400">{selectedCategory.icon}</span>
          ...
          <div className="max-h-auto overflow-y-auto">
            {categories.map((category) => (
              <button
                ...
                className={`w-full px-3 py-2 text-sm text-left flex items-center gap-3 transition-colors border-b border-gray-800/50 last:border-b-0 ${
                  value === category.id
